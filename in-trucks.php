...
<script>
    ...
    function GetDetail(str) {
        if (str.length == 0) {
            // เคลียร์ค่าเมื่อช่องป้ายทะเบียนว่างเปล่า
            ...
            xmlhttp.onreadystatechange = function () {
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    // ถ้าไม่พบข้อมูลในฐานข้อมูล (สมมติว่าส่ง JSON array ว่างเปล่ากลับมา)
                    if (myObj.length === 0) {
                        // แจ้งเตือนผู้ใช้ด้วย alert box
                        var confirmAdd = confirm("ບໍ່ພົບເລກທະບຽນໃນລະບົບ. ທ່ານຕ້ອງການເພີ່ມທະບຽນລົດໃນລະບົບບໍ່?");
                        
                        // ถ้าผู้ใช้กด OK
                        if (confirmAdd == true) {
                            // เปลี่ยนเส้นทางไปยังหน้าเพิ่มรถใหม่
                            window.location.href = "add-truck.php";
                        } else {
                            // ถ้าผู้ใช้กด Cancel ให้เคลียร์ค่าในฟอร์ม
                            document.getElementById("back_truck_plate").value = "";
                            document.getElementById("diver_name").value = "";
                            document.getElementById("truck_owner").value = "";
                        }
                    } else {
                        // ถ้าพบข้อมูล ให้เติมข้อมูลลงในช่องฟอร์ม
            ...
            xmlhttp.open("POST", "autofill_entry_trucks.php", true);
            xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xmlhttp.send("front_truck_plate=" + str);
        }
    }
</script>
